...
littered in the root of the repostiroy. One can make a repostiroy a lot more
organized by putting them in seperate folders.
...
phony. Make has a magic `.PHONY` target, all the _prerequisites_ of this target
are marked as phonies, and make will no longer check them against any file with
the same name. Instead make will assume phonies are always out of date, and need
to be recompiled. This gives us the following `Makefile`:
...
applies to the _rules_ directly referenced, and not the dependency of those
rules.
...
**Wrong!** Since `$(NAME)` is a dependency of `.PHONY`, make always considers
...
right now recompling our project using `make` won't quite work if you modified
a header file, as make is unaware of those. Running `make -B` would work, but
we can also get make to handle this automatically.
...
The `$^` and `$<` _automatic variables_ are different in a subtitle but

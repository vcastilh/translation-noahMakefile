...
info:
    title: Tutorial Makefile 2.0
    description: O novo e melhorado tutorial Makefile, para pessoas iniciantes e intermediárias. Os tópicos incluem pattern matching, compilando    de e para diretórios diferentes, header dependencies, e variáveis.
...
Os tópicos incluem pattern matching, compilando de e para diretórios diferentes,
header dependencies, e variáveis.

<p class="notice" markdown="1">

Traduções: [English](/tutorials/makefile.md) \| [français](/tutorials/makefile.fr) \| **Português (Brasil)**
<br>
...
> Um target pattern é composto de um '%' entre um prefixo e um sufixo, um deles
...
> prefixo e o sufixo é chamado stem. Assim, quando o padrão '%.o' corresponde ao
> nome do arquivo test.o, a stem é 'test'. Os pré-requisitos da pattern rule são
> transformados em nomes de arquivos reais, substituindo o '%' pela stem. Assim,
...
regras de pattern matching. `$@` relaciona-se com o _alvo_ e `$^` relaciona-se
...
> **Nota:** Você pode ter visto o operador `=` sendo utilizado no Makefiles. Também é correto, mas funciona 
...
Vamos rever nosso regra do arquivo objeto:
...
O pattern `%` não funciona somente em sufixos, ele também funciona em prefixos. Podemos
...
que utilizemos um arquivo objeto como _pré-requisito_ nós o referênciemos com seu
...
tentando usar um arquivo como `ft_obj/example.c`. O arquivo usado como entrada deve
...
Make não quer executar a regra `clean`, pois ja existe um arquivo chamado `clean`.
...
A `S^` and `S<`_automatic variables_ são diferentes de forma sútil mas importante:
